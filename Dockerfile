...
# 双加速：阿里云源 + sevencdn 双加速，安装缺失的onnx运行库
RUN sed -i 's/deb.debian.org/mirrors.aliyun.com/g' /etc/apt/sources.list.d/debian.sources && \
    apt-get update && apt-get install -y --no-install-recommends wget && \
    wget --no-check-certificate https://gh.sevencdn.com/https://github.com/microsoft/onnxruntime/releases/download/v1.14.0/libonnxruntime-linux-x64-1.14.0.tgz && \
